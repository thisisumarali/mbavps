...
import { EXECUTIVE_BODY } from "@/data/code";
  ...
  return (
    <section className="py-16 md:py-24 px-4 md:px-10 lg:px-20">
      <div className="max-w-6xl mx-auto">
        <SectionHeading
          eyebrow="Cabinet"
          title="Executive Body of Malir Bar Association"
        />
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 justify-items-center">
          {EXECUTIVE_BODY.map((member, index) => (
            <div
              key={member.name}
              className={`group flex flex-col items-center text-center ${
                EXECUTIVE_BODY.length % 4 === 2 &&
                index === EXECUTIVE_BODY.length - 2
                  ? "md:col-start-2"
                  : ""
              }`}
            >
